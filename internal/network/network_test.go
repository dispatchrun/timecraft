...
func testNamespaceExchangeDatagram(t *testing.T, ns network.Namespace, bind network.Sockaddr) {
	...
	socket1, err := ns.Socket(family, network.DGRAM, network.TCP)
	assert.OK(t, err)
	defer socket1.Close()

	socket2, err := ns.Socket(family, network.DGRAM, network.TCP)
	assert.OK(t, err)
	defer socket2.Close()
